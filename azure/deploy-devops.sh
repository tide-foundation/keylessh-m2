...
# =============================================================================
# KeyleSSH Multi-Environment Deployment Script
# Deploys: Web App + Punchd Gateway (Azure Container Instance)
#
# Environment is selected with DEPLOY_ENV (default: devops). It picks which
# azure/.env.<env>[.local] files load and the default resource names, e.g.:
#   DEPLOY_ENV=staging ./deploy-devops.sh --setup     # create staging resources
#   DEPLOY_ENV=staging ./deploy-devops.sh --gateway   # (re)deploy staging gateway
#   ./deploy-devops.sh --all                          # devops (default) full deploy
# =============================================================================
...
# Environment name selects which .env.<env>[.local] files to load and the
# default resource names. Override with: DEPLOY_ENV=staging ./deploy-devops.sh ...
# (defaults to "devops" so existing usage is unchanged)
DEPLOY_ENV="${DEPLOY_ENV:-devops}"

[ -f "$SCRIPT_DIR/.env.$DEPLOY_ENV" ] && load_env "$SCRIPT_DIR/.env.$DEPLOY_ENV"
[ -f "$SCRIPT_DIR/.env.$DEPLOY_ENV.local" ] && load_env "$SCRIPT_DIR/.env.$DEPLOY_ENV.local"

# ─── Configuration ───────────────────────────────────────────────────────────
RESOURCE_GROUP="${RESOURCE_GROUP:-KeyleSSH}"
LOCATION="${LOCATION:-australiaeast}"

# Capitalized env name for display defaults (devops -> Devops)
ENV_TITLE="$(tr '[:lower:]' '[:upper:]' <<< "${DEPLOY_ENV:0:1}")${DEPLOY_ENV:1}"

# Web App
WEBAPP_NAME="${WEBAPP_NAME:-keylessh-$DEPLOY_ENV}"
APP_SERVICE_PLAN="${APP_SERVICE_PLAN:-keylessh-plan}"
STORAGE_ACCOUNT="${STORAGE_ACCOUNT:-keylesshstorage}"
FILE_SHARE="${FILE_SHARE:-keylessh-$DEPLOY_ENV}"

# Punchd Gateway (ACI)
ACI_NAME="${ACI_NAME:-punchd-gateway-$DEPLOY_ENV}"
DNS_LABEL="${DNS_LABEL:-punchd-gateway-$DEPLOY_ENV}"
GATEWAY_IMAGE="${GATEWAY_IMAGE:-tideorg/punchd-gateway:latest}"
GATEWAY_ID="${GATEWAY_ID:-$ENV_TITLE-GW}"
GATEWAY_DISPLAY_NAME="${GATEWAY_DISPLAY_NAME:-$ENV_TITLE Gateway}"

# Signal server (must be set in .env.<env> or environment)
STUN_SERVER_URL="${STUN_SERVER_URL:?Set STUN_SERVER_URL in .env.$DEPLOY_ENV}"
API_SECRET="${API_SECRET:?Set API_SECRET in .env.$DEPLOY_ENV}"
TURN_SECRET="${TURN_SECRET:?Set TURN_SECRET in .env.$DEPLOY_ENV}"
...
usage() {
    echo "Usage: DEPLOY_ENV=<env> $0 [OPTIONS]"
    echo ""
    echo "Options:"
    ...
    echo ""
    echo "Environment: configure via azure/.env.<env> (DEPLOY_ENV, default devops)"
}
...
print_header "KeyleSSH $ENV_TITLE Deployment"
...
if [ "$DEPLOY_GATEWAY" = true ]; then
    ...
    if [ -z "$BACKENDS" ]; then
        ...
        echo "Set BACKENDS in .env.$DEPLOY_ENV, e.g.: BACKENDS='SSH Server=ssh://10.0.0.4'"
        BACKENDS="${BACKENDS:-}"
    fi
    ...
    if command -v wslpath &>/dev/null; then
        mkdir -p /mnt/c/Temp 2>/dev/null || true
        TMPFILE="/mnt/c/Temp/aci-$DEPLOY_ENV.yaml"
        TMPFILE_AZ=$(wslpath -w "$TMPFILE")
    else
        TMPFILE=$(mktemp /tmp/aci-$DEPLOY_ENV-XXXXXX.yaml)
        TMPFILE_AZ="$TMPFILE"
    fi
    cat > "$TMPFILE" <<YAML
