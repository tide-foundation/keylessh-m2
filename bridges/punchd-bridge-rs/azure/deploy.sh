...
APP_NAME="keylessh-punchd-bridge"

# Required config paths
TIDECLOAK_CONFIG="${TIDECLOAK_CONFIG:-../../data/tidecloak.json}"

echo "=== KeyleSSH Punchd Bridge (Rust) Deployment ==="
...
# Required env vars
STUN_SERVER_URL="${STUN_SERVER_URL:?STUN_SERVER_URL is required}"
...
az containerapp create \
    ...
    --cpu 0.5 \
    --memory 1Gi \
    --secrets "tidecloak-config=$TIDECLOAK_CONFIG_B64" "api-secret=$API_SECRET" "turn-secret=$TURN_SECRET" \
    --env-vars \
        "TIDECLOAK_CONFIG_B64=secretref:tidecloak-config" \
        "STUN_SERVER_URL=$STUN_SERVER_URL" \
        "API_SECRET=secretref:api-secret" \
        "TURN_SECRET=secretref:turn-secret" \
        "LISTEN_PORT=7891" \
        "HEALTH_PORT=7892" \
        "HTTPS=true" \
...
echo ""
echo "Gateway URL: https://$BRIDGE_URL"
echo ""
echo "Configure your STUN server to point gateways to this URL."
echo "The bridge will scale from 0 to 100 instances based on connections."
